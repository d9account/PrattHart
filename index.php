<?php include 'includes/pagetop_inc.php';?>

<?php include 'includes/header_inc.php';?>
<script type="text/javascript">var catPage = false;</script>
    <div class="content">
        <div class="hero"></div>

        <div class="main-category">
            <div class="mens"><a href="mens.html">MENS</a></div>
            <div class="middle"> <div class="deer"></div>
                    <div class="white-header">LOOK GREAT. FOREVER.</div>
                    <div class="box-info">
                        What good is fashion<br/>if it's not built to last?<br/>At Pratt & Hart, our<br/>reputation for timeless<br/>good looks is matched<br/>only by our tradition of<br/>quality craftmanship. 
                    </div></div>
            <div class="womens"><a href="womens.html">WOMENS</a></div>
        </div>

        <div class="clear"></div>

        <div class="sub-category link-section">
            <div class="winter">
            	<div class="link-section-wrapper">
            		Winter <div class="sm-spacer"></div>
            		<a href="mens-winter.html">MENS</a><span>|</span><a href="womens-winter.html">WOMENS</a>
            	</div>
            </div>
            <div class="middle"></div>
            <div class="driving">
            	<div class="link-section-wrapper">
            		Driving <div class="sm-spacer"></div>
            		<a href="mens-driving.html">MENS</a><span>|</span><a href="womens-driving.html">WOMENS</a>
            	</div>
            </div>

        </div>

         <div class="clear"></div>

        <div class="small-ad">
            
            <div class="nav-link" onclick="window.location='mens.html'"></div>

        </div>
         <div class="clear"></div>

        <div class="where-to-buy link-section">
            <div class="touchscreen">
            	<div class="link-section-wrapper">
            		Touchscreen <div class="sm-spacer"></div>
            		<a href="mens-touchscreen.html">MENS</a><span>|</span><a href="womens-touchscreen.html">WOMENS</a>
            	</div>
            </div>
            <div class="middle">
            
             <div class="white-header">WHERE TO BUY</div>
                    <div class="box-info">
                    
                       Find Prett & Hart gloves at<br/>fine retailers across the<br/>country, or shop now at<br/><a href="http://www.leatherglovesonline.com" class="box-link">LeatherGlovesOnline.com</a>
                        <div class="white-spacer"></div>
                        <a href="retailer.html">Find a Retailer</a><a href="shop.html">Shop Now</a>
                    </div>

            </div>
            <div class="mittens">
            	<div class="link-section-wrapper">
            		Mittens <div class="sm-spacer"></div>
            		<a href="womens-mittens.html">WOMENS</a>
            	</div>
            </div>
        </div>
         <div class="clear"></div>
    </div>

</div>

<?php include 'includes/footer_inc.php';?>
<?php include 'includes/pagebottom_inc.php';?>